...
# Saison-Namen
# Saison 2024/2025 ist jetzt Teil der Trainingsdaten (Zielsaison ist 2025/26)
season <- c("2012/2013", "2013/2014", "2014/2015", "2015/2016", "2016/2017",
            "2017/2018", "2018/2019", "2023/2024", "2024/25")
n_seasons <- length(season)
# Nur ganze Saisons (24 Wochen nach KW 40, außer 2015), kein Covid
# Cutoff verschiebt sich auf den Beginn der neuen Zielsaison 2025/26
df_train <- df %>% filter(week < 19 | week > 40) %>% 
                     filter(date != "2015-10-11") %>%
                     filter(date < "2019-10-13" | date > "2023-05-10") %>%
                     filter(date < "2025-10-12")
...
# Testdaten - Zielsaison 2025/26
df_test <- df %>% filter(date >= "2025-10-12" & date <= "2026-05-03")
...
step6_sample_forecasts <- function(y_obs, # Beobachtete Counts [T]
...
run_inferno_multi <- function(grouped_list, last_obs_weeks = 1:30, 
  ...
  for(k in last_obs_weeks){
    ...
    res_k <- run_inferno_parallel(grouped_k, n_cores = n_cores,
...
# Ausführen (mit allen Daten aus Saison 25/26)

results_multi <- run_inferno_multi(age_grouped_list, n_cores = 3,
...
all_horizons <- do.call(rbind, lapply(names(results_multi), function(wk){
  res_k <- results_multi[[wk]]
...
week_updates$Woche[1] <- as.Date("2025-10-12")
for(i in 2:30){
...
write.csv(inferno_long_final, "Inferno_long_final_26_1.csv")
