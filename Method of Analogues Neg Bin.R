...
fit_psi <- function(df_name){
  df_psi_help <- get(df_name)
  model <- glm.nb(observed ~ -1 + offset(forecast), link = identity, data = df_psi_help)
  return(model$theta)
}

names_psi_help <- c(paste0("pred_all_", 1:4),
...
psis_est <- sapply(names_psi_help, fit_psi)

psis_df_est <- data.frame( modell = names_psi_help, psi_est = psis_est)
...
psis_lookup <- psis_df_est |>
  ...
  dplyr::select(age_group, horizon, psi = psi_est)

# Join + Intervalle berechnen
future_intervals_df <- future_predictions_df |>
  left_join(psis_lookup, by = c("age_group", "horizon")) |>
  mutate(
    # 50%-Intervall
    lower_50 = qnbinom(0.25, size = psi, mu = forecast),
    upper_50 = qnbinom(0.75, size = psi, mu = forecast),
    # 80%-Intervall
    lower_80 = qnbinom(0.1, size = psi, mu = forecast),
    upper_80 = qnbinom(0.9, size = psi, mu = forecast),
    # 95%-Intervall
    lower_95 = qnbinom(0.025, size = psi, mu = forecast),
    upper_95 = qnbinom(0.975, size = psi, mu = forecast)
  )
...
write.csv(moa_long_final, "MoA_long_final.csv")
